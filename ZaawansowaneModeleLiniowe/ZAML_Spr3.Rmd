...
```{r data_overwiev}
shop_data <- read.csv("C:\\Users\\Blazej Wiorek\\Desktop\\RProjects\\ZaawansowaneModeleLiniowe\\shop_data.csv")
...
```{r boxplot_1}
clients_day_histogram <- ggplot(shop_data, aes(x=day, y=no.klients)) + 
                         geom_boxplot() + 
                         ggtitle("Amount of clients relative to weekdays")

plot(clients_day_histogram)
```

W przeciwieństwie do dni tygodnia wygląda na to, że fakt, że w danym dniu odbyło sie wydarzenie sportowe nie ma znaczącego wpływu na liczbę klientów. 
```{r boxplot_2}
clients_event_histogram <- ggplot(shop_data, aes(x=events, y=no.klients)) + geom_boxplot() + ggtitle("Amount of clients relative to sport events")
plot(clients_event_histogram)
```

Kolejny wykres przedstawia rozkład klientów w poszczególych godzinach pracy sklepu. Ze zrozumiałych powodów pora dnia istotnie różnicuje liczbę klientów w sklepie. Jest to kolejny wykres sugerujący, że można wprowadzić grupowanie. Można to zrealizować np. ze względu na godzinny poranne [8, 11], godziny popołudniowe [12, 15], godziny wieczorne [16, 19] oraz godziny nocne [20, 23]. Pomysł ten został zrealizowany w zadaniu 4.
```{r boxplot_3}
clients_hour_histogram <- ggplot(shop_data, aes(x=hour_fct, y=no.klients)) + geom_boxplot() + ggtitle("Amount of clients relative to hours")
plot(clients_hour_histogram)
```

Można się zastanowić czy nie mam tutaj do czynienia ze zjawiskiem nadmiernej dyspersji szczególnie w kontekście liczby klientów pomiędzy godziną 16 a 19. Na podstawie średniej i wariancji (obliczone niżej) można wnioskować, że faktycznie wspomniany problem pojawia się w przypadku tego zbioru danych. Na chwile obecną nie podjęte zostaną żadne kroki mające na celu rozwiązanie tego problemu, warto jednak mieć to w pamięci. 
```{r overdispersion}
clients_by_hours <-shop_data %>% dplyr::select(no.klients, hour) %>% filter(hour >=16 & hour <= 19)
paste("Mean: ", mean(clients_by_hours$no.klients), ", Std: ", sd(clients_by_hours$no.klients)^2)
```
...
```{r summary_plot}
summary_plot <- qplot(hour_fct, no.klients, shape = as.factor(events),col = day, data = shop_data) + ggtitle("Number of clients - summary plot")
plot(summary_plot)
```

Wykres poniżej umacnia w przekonaniu, że wydarzenia sportowe nie maja wpływu na liczbę klientów. 
```{r importance_of_events}
summary_plot_2 <- qplot(hour_fct, no.klients, facets = events ~ day, data = shop_data) + ggtitle("Importance of events")
plot(summary_plot_2)
```

Dwa wykresy poniżej nie wnoszą nowych informacji jednak zdecydowałem się dołączyć na rzecz kompletności raportu. 
```{r example_plot_1}
summary_plot_3 <- qplot(hour_fct, no.klients, color = day, data = shop_data)
plot(summary_plot_3)
```

```{r example_plot_2}
summary_plot_4 <- qplot(hour_fct, no.klients, facets = ~day, data = shop_data)
plot(summary_plot_4)
```

## Zadanie 3

w zadaniu mowa jest o tym, że wszystkie zmienne objaśniające mają być zakodowane jako factor. Warto sie więc upewnić, że dane spełniają to założenie. 
```{r check_if_factors}
...
lapply(shop_data_fct, amount_of_levels)
```
...
plan_matrix <- data.frame(subgroups=as.vector(subgroups), clients=as.vector(clients), lin_pred=lin_pred, coef_val=coef_val)
plan_matrix
```

Należy zwrócić uwagę na to, że zgodnie z oczekiwaniami współczynniki odpowiadające sobocie i niedzieli są porównywalne. W przypadku dni roboczych obserwowana jest większa wariancja. Dodatkowo jako baseline przyjęta została wartość odpowiadająca weekendowi w godzinach porannych ([8, 12)).
...
paste("P-value for Wald test: ", p_val[1, 1])
```
...
pred_df["client_per_worker"] <- ceiling(pred_df["clients_pred"] / 20)
pred_df
```

Kolumna clients_pred zawiera szacowaną liczbę klientów w ciągu godziny w zadanym bloku czterogodzinnym. kolumna client_per_worker to liczba pracowników w bloku czterogodzinnym którzy są potrzebni do obsługi zadanej liczby klientów. Jeśli założyć, że pracownik nie może pracować więcej niż 8h dziennie to można zaproponować gólny grafik w następujący sposób: 
