---
title: "Analiza dużych zbiorów danych 1"
...
Z przedstawionego podsumowania można wywnioskować, że przyjmując poziom istotności na poziomie $\alpha=0.01$, wszystkie zmienne objaśniające zawarte w modelu są istotne (brak podstaw do odrzucenia $H_0$).
...
Model wykorzystujący probit jako link function charakteryzuje się najniższą wartością AIC oraz Deviance. Na tej podstawie mogę określić, że jest to model charakteryzujący się najlepszym dopasowaniem = najmniejszym błędem.  

Ogólnie warto zauważyć, że wszystkie wartości są do siebie zbliżone, z czego wynika, że przedstawione modele niewiele się od siebie różnią. Aby zwizualizować różnicę postanowiłem przedstawić wykresy poszczególnych funkcji:
```{r LinkFunctionsComparison}
...
colnames(link_funcs_ds.melted) <- c("x", "Linkage", "Probability")

p <- ggplot(link_funcs_ds.melted, aes(x=x, y=Probability)) +
     geom_line(aes(color=Linkage))
plot(p)
```
...
Zastanówmy się teraz nad dwoma miarami oceny skuteczności klasyfikatora binarnego:
$$
