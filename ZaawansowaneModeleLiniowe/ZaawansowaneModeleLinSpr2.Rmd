...
show_estimates_histogram <- function(estimates_df) {
...
bias_estimation <- function(estimates_df) estimates_df %>% select(starts_with("beta"))-3
variance_estimation <- function(estimates_df) estimates_df %>% select(starts_with("beta")) %>% var()
```

### Podpunkt a)

```{r histograms_coefficients}
estimates_res <- experiment(400, 3)
estimates_df <- estimates_res$estimates
show_estimates_histogram(estimates_df)
```
...
```{r bias_estimation}
estimated_bias <- bias_estimation(estimates_df)
summary(estimated_bias)
```
...
```{r variance_estimated}
variance_estimation(estimates_df)
```

```{r true_variance}
estimates_res$cov_matrix
```
